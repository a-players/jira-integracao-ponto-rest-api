...
export default function TblUnderHoursReport(props) {
  ...
  const headers = [
    ...
    { id: 1, dataType: "String", name: props.texts && props.texts['relatorio.hours.page.user'] ? props.texts['relatorio.hours.page.user'] : 'Employee', value: 'employee'},
    ...
    { id: 4, dataType: "Float", name: props.texts && props.texts['relatorio.hours.page.requiredhours'] ? props.texts['relatorio.hours.page.requiredhours'] : 'Required Hours', value: 'requiredhours'},
    { id: 5, dataType: "Float", name: props.texts && props.texts['relatorio.hours.page.submittedhours'] ? props.texts['relatorio.hours.page.submittedhours'] : 'Submitted Hours', value: 'submittedhours'},
    { id: 6, dataType: "Float", name: props.texts && props.texts['relatorio.hours.page.hoursponto'] ? props.texts['relatorio.hours.page.hoursponto'] : 'Check-in Hours', value: 'hoursponto'},
    { id: 7, dataType: "String", name: props.texts && props.texts['relatorio.hours.page.reason'] ? props.texts['relatorio.hours.page.reason'] : 'Comment', value: 'reason'},
    { id: 8, dataType: "Date", name: props.texts && props.texts['relatorio.hours.page.submitteddatetime'] ? props.texts['relatorio.hours.page.submitteddatetime'] : 'Submitted Date and Time', value: 'submitteddatetime'},
  ]
    ...
    useEffect(() => {
        setSubmittedList(props.data.partialTimesheetReportsDTOList);
    }, [props])
    ...
    return (
        <div>
            ...
            <table id="tblUnderHoursReport" className="pages-tbl">
                <thead>
                  <h2>{props.texts && props.texts['relatorio.hours.page.partialts'] ? props.texts['relatorio.hours.page.partialts'] : 'Partial Submitted Timesheets'}</h2>
                  <tr>
                    {headers.map((item) => {    
                      return hasToShow(item.id) && (
                        <th key={item.id} className={`sortable ${sortColumn === item.id ? 'sorted' : ''}
                            ...
                            onClick={() => sortTableUnd(item.id, item.dataType)}
                        >
                          {item.name}
                          {sortColumn === item.id && (
                            <>
                            ...
                            submittedList.map((item) => {
                                return (
                                    <tr key={item.id}>
                                        ...
                                        { hasToShow(4) && ( <td>{Number(item.requiredHours).toFixed(2)}</td> ) }
                                        { hasToShow(5) && ( <td>{Number(item.submittedHours).toFixed(2)}</td> ) }
                                        { hasToShow(6) && ( <td>{((item.horasPonto == '?') ? item.horasPonto : Number(item.horasPonto).toFixed(2))}</td> ) }
                                        { hasToShow(7) && ( <td>{item.reason}</td> ) }
    ...
    function sortTableUnd(n, dataType) {
      utils.showBlock(true);
      console.log("Ordenando: " + n);
      console.log("DataType: " +dataType);
      console.log("Carregando");
      var table, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0;
      table = document.getElementById("tblUnderHoursReport");
      switching = true;
      dir = "asc";
